...
export default function Header() {
  ...
  return (
    <div className='flex justify-between items-center backdrop-blur-md text-white p-7'>
        ...
        <div className='w-[150px] sm:w-[200px] md:w-[400px] bg-white p-3 rounded-lg text-black flex items-center transition-all duration-300 hover:scale-105'>
        ...
        <div className='flex items-center gap-2 flex-col sm:flex-row text-lg sm:text-2xl'>
            <Link href={'/login'} className="transition-all duration-300 hover:scale-110 hover:-translate-x-2">Log in</Link>
            |
            <Link href={'/register'} className="transition-all duration-300 hover:scale-110 hover:translate-x-2">sign up</Link>
        </div>
    </div>
  )
}
